...
export const CharacterManager: React.FC = () => {
  ...
  const [expandedSettingCollectionIds, setExpandedSettingCollectionIds] =
  ...
  const toggleSettingCollectionExpanded = (id: string) => {
    setExpandedSettingCollectionIds((prev) => {
        ...
        {characters.length === 0 ? (
          ...
          <div className="space-y-3">
            {characters.map((c, index) => {
              ...
              const settingCollectionExpanded =
                expandedSettingCollectionIds.has(c.id)
                    ...
                    <div className="border-t px-4 pb-4 pt-3 space-y-3">
                      ...
                      <div className="rounded-md border border-amber-100 bg-amber-50/50 p-3">
                        ...
                        {editingSettingCollectionId === c.id ? (
                          ...
                          <div>
                            <p
                              className={`min-h-[72px] whitespace-pre-wrap text-sm leading-relaxed text-gray-700 ${
                                c.setting_collection &&
                                !settingCollectionExpanded
                                  ? 'max-h-[14.25rem] overflow-hidden'
                                  : ''
                              }`}
                            >
                            ...
                            {c.setting_collection && (
                              <Button
                                variant="ghost"
                                size="sm"
                                className="mt-2 h-7 px-2 text-amber-700 hover:bg-amber-100"
                                onClick={(e) => {
                                  e.stopPropagation()
                                  toggleSettingCollectionExpanded(c.id)
                                }}
                              >
                                {settingCollectionExpanded ? (
                                ...
                                {settingCollectionExpanded ? '收起' : '展开'}
                              </Button>
                            )}
                          </div>
                        )}
