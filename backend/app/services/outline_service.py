...
class OutlineService:
    ...
    async def expand_node(
        ...
        children_data = []
        for batch_start in range(0, count, OUTLINE_EXPAND_BATCH_SIZE):
            batch_count = min(OUTLINE_EXPAND_BATCH_SIZE, count - batch_start)
            messages = [
                {"role": "system", "content": system_prompt},
                {
                    ...
                    "content": OUTLINE_EXPAND_USER.format(
                        ...
                        siblings_info=self._build_expand_siblings_info(
                            params.get("siblings_info", "无"), children_data
                        ),
                        expand_request=self._build_expand_request(
                            params.get("request", "请自然扩展"),
                            count,
                            batch_start,
                            batch_count,
                        ),
            ...
            response = await llm_orchestrator.chat(
                ...
                max_tokens=params.get("max_tokens") or OUTLINE_EXPAND_MAX_TOKENS,
                **json_object_response_kwargs(),
            )
            batch_children = self._extract_expand_children(response)
            children_data.extend(batch_children[:batch_count])
            if len(children_data) >= count:
                children_data = children_data[:count]
                break
    ...
    @staticmethod
    def _build_expand_request(
        request: str,
        total_count: int,
        batch_start: int,
        batch_count: int,
    ) -> str:
        base_request = request or "请自然扩展"
        if total_count <= OUTLINE_EXPAND_BATCH_SIZE:
            return base_request
        start = batch_start + 1
        end = batch_start + batch_count
        return (
            f"{base_request}\n"
            f"本次是批量扩展中的第 {start} 到 {end} 个子节点，"
            f"总共需要 {total_count} 个子节点。请保持与前后章节顺序连贯，"
            "避免与已生成子节点重复。"
        )

    @staticmethod
    def _build_expand_siblings_info(
        siblings_info: str,
        generated_children: list[dict],
    ) -> str:
        generated_titles = [
        ...
        if not generated_titles:
            return siblings_info or "无"
        generated_info = "本次扩展已生成子节点：" + "、".join(generated_titles)
        if siblings_info and siblings_info != "无":
            return f"{siblings_info}\n{generated_info}"
        return generated_info

    @staticmethod
    def _extract_expand_children(response: str) -> list[dict]:
        children_data = json.loads(response)
        children = (
            children_data
            if isinstance(children_data, list)
            else children_data.get("children") or []
        )
